    ...
    ajax({
        ...
        success: function (data) {            
            ...
            data.nameFile.forEach(function (c, i) {
                ...
                bag(d).addHtml('<div class="thumbnail"><img src="' + c + '"><div class="caption"><p></p></div></div>');
...
function addNewImages() {
    ...
    { "id": "imgName", "type": "text", "name": "Сохранить как в /images/"+urlActiv, "value": "" }]);
...
function saveNewImages() {
    ...
    model.append("dirName", "/images/" + urlActiv + "/");
    model.append("imgName", bag('#imgName').val());
    ajax({
        url: '../userImages/saveNewImages',
        data: model,
        success: function (data) {
...
function startRedactor() {
    ...
    document.querySelector('#parentContainer').querySelectorAll('[data-content]').forEach(function (data, i) {        
...
var bagfield = {
    ...
    createSelect: function (idField, valueArray) {
        ...
        array.forEach(function (data, i) {
            field.insertAdjacentHTML('beforeend', '<option value="' + data.value + '">' + (data.name ? data.name : data.value) + '</option>');
        });
        return field;
    },
